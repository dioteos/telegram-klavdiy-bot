Сесію запущено 🟢

Memory: {memory_count} файлів завантажено
Tasks: {tasks_found} знайдено / {tasks_registered} зареєстровано / {tasks_skipped} пропущено

Зареєстровані задачі:
{registered_list}

Пропущено:
{skipped_list}

<!-- Format guide (not sent to Telegram):
  {memory_count}     — integer, e.g. "13"
  {tasks_found}      — integer, e.g. "3"
  {tasks_registered} — integer, e.g. "2"
  {tasks_skipped}    — integer, e.g. "1"
  {registered_list}  — bullet list: "• task-name — HH:MM" per line
  {skipped_list}     — bullet list: "• task-name (reason)" per line
  If {tasks_skipped} is 0, omit the "Пропущено:" section entirely.
-->
